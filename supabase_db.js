...
//
//  Problem jo fix ki:
//  - Data Supabase se load hota tha but app variables update nahi hote the
//  - Timing issue: variables exist nahi karte the load ke time
//  - Ab: DB load hone ke baad app ko forcefully refresh karta hai
// ================================================================
...
const TABLES = {
  ...
  'hops-employees': {
    table: 'employees',
    pack:   o => ({ id: o.id, name: o.name||'', dept: o.dept||'', designation: o.designation||'', email: o.email||'', password: o.password||'' }),
    unpack: r => ({ id: r.id, name: r.name||'', dept: r.dept||'', designation: r.designation||'', email: r.email||'', password: r.password||'' }),
  },
...
//  RETRY logic: agar variable abhi exist nahi karta, wait karo
// ================================================================
function updateAppVariable(key, data) {
  ...
  // Direct assign agar variable exist karta hai
  if (typeof window[varName] !== 'undefined') {
    window[varName] = data;
    return;
  }

  // Variable abhi nahi hai — retry karo 10 baar (100ms gap)
  let tries = 0;
...
// ================================================================
//  ld() — LOAD FUNCTION (localStorage se padho)
// ================================================================
...
window.sv = async function(key, val) {
  ...
  // Step 2: Supabase mein save (agar ready hai)
  if (!_ready || !_sb) return;

  // USER LINKS special handling
  if (isLinkKey(key)) {
    if (!Array.isArray(val)) return;
    const username = linkUsernameFromKey(key);
    try {
      // Sirf insert/upsert — delete automatic nahi hoga
      if (val.length > 0) {
        ...
        else console.log('✅ Links saved ['+key+'] —', val.length);
      }
  ...
  try {
    // Sirf upsert karo — delete kabhi automatic nahi hoga
    // Delete sirf Supabase dashboard se manually karna hoga
    if (val.length > 0) {
...
// ================================================================
//  setupRealtime() — Live changes dikh'en
// ================================================================
function setupRealtime() {
  const realtimeTables = ['tasks', 'issues', 'departments', 'employees', 'delegations'];
...
// ================================================================
//  INIT — Page load hote hi shuru hota hai
// ================================================================
(async function startDB() {
  ...
  try {
    ...
    // Step 7: App refresh karo fresh data ke saath
    if (typeof currentRole !== 'undefined' && currentRole) {
      // User pehle se login hai — page aur badges dono refresh karo
      if (typeof renderPage === 'function') renderPage(currentPage);
      if (typeof updateBadges === 'function') updateBadges();
      if (typeof buildSidebar === 'function') buildSidebar();
    } else {
      // Session restore karo
...
//  type → 'task','issue','employee','dept','admin','handover'
// ================================================================
